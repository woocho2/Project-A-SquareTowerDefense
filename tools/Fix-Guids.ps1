# ==============================================================================
# Fix-Guids.ps1
# Ensures all GUIDs are exactly 32 hex characters for Unity compatibility
# ==============================================================================
...
foreach ($cfg in $configs) {
    ...
    # Exactly 32 characters (8 + 20 + 4 = 32)
    $guidPng        = "7a070${id}00000000000000000000${id}b"
    $guidAnim       = "7a070${id}00000000000000000000${id}a"
    $guidController = "7a070${id}00000000000000000000${id}c"

    if ($guidPng.Length -ne 32 -or $guidAnim.Length -ne 32 -or $guidController.Length -ne 32) {
        throw "GUID length error: $($guidPng.Length)"
    }
    
    $firstSpriteId = [int]("${id}100")

    # 1. Texture Meta in Assets/4. DotAsset/3. Projectile/
    $metaContent = @"
fileFormatVersion: 2
guid: $guidPng
TextureImporter:
  ...
  assetBundleVariant: 
"@
    Set-Content -Path "Assets/4. DotAsset/3. Projectile/${fb}.png.meta" -Value $metaContent -Encoding UTF8
...
"@
    ...
    Write-Output "Fixed GUIDs (32-char) for ${id}"
}

Write-Output "All GUIDs successfully verified and fixed to 32 characters!"
