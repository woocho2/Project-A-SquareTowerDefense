...
$emblemPath = "d:/MyGitHub/ProjectA/Project-A-SquareTowerDefense/Assets/4. DotAsset/2. Tower/3. Emblem/Emblem/13. Dark.png"
...
# Transform point into tilted ellipse space
# Tilted at angle -22 degrees, aspect ratio 1.5 : 1
function Get-TiltedPoint([float]$cx, [float]$cy, [float]$r, [float]$ang, [float]$tiltRad = -0.38, [float]$aspectY = 0.65) {
    # Unrotated elliptical coords
    $ux = $r * [Math]::Cos($ang)
...
# Generate 24 infalling particle streams across frames 0..4
$particles = @()
for ($i = 0; $i -lt 24; $i++) {
...
for ($f = 0; $f -lt 10; $f++) {
    ...
    # -------------------------------------------------------------
    # 1. Outer Gravitational Distortion Halo (부드러운 방사 왜곡 아우라)
    # -------------------------------------------------------------
    if ($f -le 5) {
        $haloProgress = $f / 5.0
        $haloAlpha = [int](70 * [Math]::Sin(($haloProgress + 0.1) * [Math]::PI * 0.9))
        $haloR = (50.0 - $f * 3.5) # Warps inward
    ...
    # -------------------------------------------------------------
    # 2. Infalling Suction Streams & Particles (중심으로 휘감겨 빨려들어가는 부드러운 나선 궤적)
    # -------------------------------------------------------------
    foreach ($pt in $particles) {
        $age = $f - $pt.Birth
        if ($age -ge 0 -and $age -le 3) {
            $tau = $age / 3.0 # 0.0 -> 0.33 -> 0.66 -> 1.0

            # Calculate 5 spline sample points from (tau - 0.45) to tau
            $ptsArray = @()
            for ($s = 0; $s -le 4; $s++) {
            ...
            # Color gradient along accretion
            ...
            # Draw curved suction streak with round caps
    ...
    if ($f -le 6) {
        ...
        # Dynamic parameters for suction and implosion
        $scale = 1.0
        $rotDeg = 0.0
        $alphaMul = 1.0
        ...
        switch ($f) {
            ...
            4 { $scale = 0.85; $rotDeg = 185.0; $alphaMul = 1.00 } # Tightening suction!
            5 { $scale = 0.58; $rotDeg = 260.0; $alphaMul = 0.92 } # Gravitational implosion!
            6 { $scale = 0.28; $rotDeg = 330.0; $alphaMul = 0.70 } # Singularity collapse!
        }
        ...
        # Event horizon core inside the rotated coordinate system
        if ($f -ge 2 -and $f -le 4) {
            ...
            # Glowing incandescent photon ring
    ...
    # -------------------------------------------------------------
    # 4. Singularity Flash (F6) - 붕괴 순간의 특이점 섬광
    # -------------------------------------------------------------
    if ($f -eq 6) {
        ...
        # Pure black singularity center dot
    ...
    # -------------------------------------------------------------
    # 5. Gravitational Wave Recoil Pulse (F7..F9) - 시공간 리플 & 소멸
    # -------------------------------------------------------------
    if ($f -ge 7) {
        $waveProgress = ($f - 6) / 3.8 # 0.26, 0.52, 0.79
        $waveR = 24.0 + ($waveProgress * 30.0) # Expands 31 -> 48
        $waveAlpha = [int]([Math]::Max(15.0, 210 * (1.0 - $waveProgress)))

        # Tilted elliptical shockwave
...
$outPath = "C:/Users/user/.gemini/antigravity/brain/dc024a25-2f90-4a0d-912c-bd52cd35c0ed/test_dark_blackhole_sheet.png"
$sheet.Save($outPath, [System.Drawing.Imaging.ImageFormat]::Png)

# Generate Showcase Banner (Side-by-side frames + 2x Zoom of F2 & F3)
$bannerW = 1200
$bannerH = 500
...
$bgfx.DrawString("13. Dark.png 문양의 나선 소용돌이 형태 + 중심으로 빨려 들어가는 대칭 물질 흡입 스트림(Infalling Accretion) + 특이점 붕괴", $fontSub, $bGray, 30, 48)
...
# Row 2: 2.5x Zoom Comparison of Peak Suction (F1, F2, F3, F4, F6)
$zoomFrames = @(
...
$bannerPath = "C:/Users/user/.gemini/antigravity/brain/dc024a25-2f90-4a0d-912c-bd52cd35c0ed/preview_dark_blackhole_suction.png"
...
Write-Host "Generated Dark Black Hole Suction Test successfully!"
